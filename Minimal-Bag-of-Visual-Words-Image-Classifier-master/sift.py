...
def process_image(imagename, resultname='temp.sift', dense=False):    
    ...
    if dense == False:
        ...
        with open(resultname, 'wb') as f:
            pickle.dump([locs.T, descriptors.T], f, protocol=pickle.HIGHEST_PROTOCOL)
...
def read_features_from_file(filename='temp.sift', dense=False):
    ...
    if dense == True:
        with open(filename, 'rb') as f:
            locs, descriptors =pickle.load(f)
    else:           
        f = open(filename, 'r')
...
def match(desc1, desc2):
    ...
    for i in range(desc1_size[0]):
        ...
        indx = argsort(math.arccos(dotprods))
        
        #check if nearest neighbor has angle less than dist_ratio times 2nd
        if math.arccos(dotprods)[indx[0]] < dist_ratio * math.arccos(dotprods)[indx[1]]:
