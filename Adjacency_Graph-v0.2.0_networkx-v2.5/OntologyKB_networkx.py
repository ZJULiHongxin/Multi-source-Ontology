...
class KnowledgeGraph:
    ...
    def __init__(self, KBpath):
        ...
        print('\033[1;33m Initializing hand detector. Please cover the squares with your hands... \033[0m')
        self.hd = Recognizer.handDetector(cameraNo=0, sample_time=1)
        self.hd.initDetector()
        self.cur_frame = []
        self.handPosition = 0

        self.val_map = {'Tool': 1.0, 'Motion': 0.8, 'Fruit': 0.3, 'Vegetable': 0.3, 'Sauce': 0.4, '': 0.5, None: 0.5}
    ...
    def initSession(self):
        """ Activate and Initialize an interaction session with ontology knowledge base. """
        while True:
            # Enquiring...
            print(
                "\033[1;33m Please select a scenario ... \n ( Say 'new kitchen' to new a knowledge base for kitchen situation)\n ( Say 'open kitchen' to open a existing knowledge base)\n  \033[0m")
            if self.mode == 'Y':
                order = (input_voice_secure(ARGS, model, "waiting for order...")).strip().lower()
            else:
                order = (input("waiting for order...")).strip().lower()
            ...
            if self.g is not None:
                print("\033[1;34m [Note] Knowledge base opened successfully...\n \033[0m")
    ...
    def draw_graph(self):
        ...
        #         print(node[1]['node_prop'].name, node[1]['node_type'], self.val_map.get(node[1]['node_type'], 0.6))

        values = [self.val_map.get(node[1]['node_type'], 0.6) for node in self.g.nodes(data=True) if
                  isinstance(node[0], str)]
        pos = nx.circular_layout(self.g)
        nx.draw_networkx(self.g, pos, edgelist=self.g.edges(), label=labels, cmap=plt.get_cmap('Set3'),
                         node_color=values)
    ...
    def update_object_node(self, node):
        ...
        if name in graph_keys:
            ...
            # otherwise this node will be replaced with the identical node which has been obtained before
                ...
                img_path = os.path.join(img_path, objnode_list[search_idx].name + '_' + str(
                    len(objnode_list[search_idx].img_path)) + '.jpg')
    ...
    def testThreading(self):
        ...
        try:
            while True:
                ...
                print("""\033[1;33m Describe a process: 
    ...
    \033[0m""")
                ...
                if self.mode == 'N':
                    ...
                    order = (input_voice_secure(self.ARGS, self.model, 'Waiting for order...')).strip().lower()
                ...
                if match is not None:
                    ...
                    while True:
                        # requiring...
                        if self.mode == 'N':
                            description = input('\033[1;33m Please decribe it (say "ok" to finish it)... \033[0m').lower()
                            ...
                            description = (input_voice_secure(self.ARGS, self.model, \
                                                              '\033[1;33m Please decribe it... \033[0m')).lower()
                        ...
                        if prop1 is not None or prop2 is not None:
                            contents = description.title().split()
                            contents = contents[1:]
                            print("[Note] ok! its contents have been updated:")
                            print(contents,'\n')
                            continue
                        ...
                        if prop1 is not None or prop2 is not None:
                            while 1:
                                ...
                                if self.mode == 'N':
                                    s = input(
                                        '\033[1;33m Please describe its state (say "done" to finish it)... \033[0m').lower()
                                else:
                                    # Speech Recognition Here!
                                    ...
                                    s_single = (input_voice_secure(self.ARGS, self.model,
                                                                   '\033[1;33m Please describe its state... \033[0m')).lower()
                ...
                """
                Print out the details of every node in a form of text
                """
                match = re.match('details', order)
                if match is not None:
                    for key in self.g.graph['node_number_dict'].keys():
                        lst = self.g.graph['node_number_dict'][key]
                        for i in lst:
                            print(i)
                    continue

                """
                Save the knowledge graph and terminate the whole progam
                """
                    ...
                    if 'use' in order:
                        ...
                        if idx == len(process_objectlist):
                            if obj_name in self.g.graph['node_number_dict'].keys():
                                ...
                                print('[Note] The object being operated "{}" was obtained long ago'.format(
                                    previous_objnode.name))
                            else:
                                print(
                                    '\033[0;31m[Warning] The result node "{}" cannot be found in the graph. Please describe it first.\n \033[0m'.format(
                                        obj_name))
                                continue
                        ...
                        if not find_flag:
                            print(
                                '\033[0;31m[Warning]The tool node "' + tool_name + '" cannot be found! Please describe it first.\n \033[0m')
                            continue
                        ...
                        while True:
                            command = (
                                input("\033[1;33m [Note] Say 'finish' to end recording your action...\n \033[0m")).lower()
                            # Speech Recognition
                        ...
                        print("\033[1;33m Please check the results below..\n \033[0m")
                ...
                if match is not None:
                    ...
                    print('\033[1;34m[Note] Found matched image: \033[0m', end='')
                    ...
                    print('\033[1;34m This object is \033[0m' + obj_name)
                ...
                """
                Search for a path that can lead to the target node.
                """
                match = re.match('how', order)  # how to make apple_slices.
                if match is not None:
                    ...
                    while True:
                        if order[last_space] == ' ':
                            break
                        last_space -= 1
                    obj_name = order[last_space + 1:-1]

                    # find all nodes with the same name
                    try:
                        all_obj_with_the_same_name = self.g.graph['node_number_dict'][obj_name]
                    except:
                        print("There is no such object {} in this knowledge base.")
                        continue

                    lst = []
                    for obj in all_obj_with_the_same_name:
                        prerequisite, subgraph = self.DFSTraverse(obj.name)
                        lst.append(subgraph)
                        print(prerequisite)

                    row_num = floor(len(lst) ** 0.5)
                    col_num = len(lst) // row_num + 1 if row_num > 1 else len(lst)
                    ...
                    for i in range(row_num):
                        for j in range(col_num):
                            ...
                            if idx < len(lst):
                                nx.draw_networkx(lst[idx], with_labels=True)
                                plt.axis("off")
                                idx += 1
    ...
    def DFSTraverse(self, node):
        process = nx.DiGraph(title=node)
        process.add_node(node)
        visited_dict = {node: None}
        path = [node]
        self.DFS(node, visited_dict, path, process)
        return path,process

    def DFS(self, node, visited_dict, path, process):
        """ Implement DFS of the unvisited portion of graph g starting at vertex u."""
        for parent in self.g.predecessors(node):
            if parent not in visited_dict:
                path.append(parent)
                process.add_node(parent)
                process.add_edge(parent,node)
                visited_dict[parent] = self.g.edges([node,parent])
                self.DFS(parent, visited_dict, path, process)
