...
# Use VAD(voice activity detection) to improve performance
...
def init_Speechrec():
    ...
    parser.add_argument('-m', '--model', default='E:\\github\\Multi-source-Ontology\\Adjacency Graph-v0.1.1\\SpeechR\\deepspeech-0.6.1-models',
...
def input_voice(ARGS, model, info_print=None):
    if not info_print == None:
        print(info_print)
    ...
    for frame in frames:
        if frame is not None:
            ...
            logging.debug("Recognized successfully.")
            return text
            #stream_context = model.createStream()

def recheck(words):
    while True:
        print("Is expected input: ", words, "?")
        op = input("y/n")
        if op == 'y':
            flag = 1
        elif op == 'n':
...
def input_voice_secure(ARGS, model, info_print=None):
    while True:
        words = input_voice(ARGS, model, info_print)
        flag = recheck(words)
        if flag == 0:
            continue
        else:
...
#
# input_voice(ARGS, model, "HAHAHAHAHA")
# input_voice(ARGS, model)